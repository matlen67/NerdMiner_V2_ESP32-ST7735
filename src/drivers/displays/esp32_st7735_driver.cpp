// *************************************************************
// Wiring 
// AZ-Delivery 1,8 inch SPI TFT Display (controller ST7735)
// ESP32 DevKit C V4
//
// ESP32	 1,8" TFT
//  5V  ---- VCC
...
#include <TFT_eSPI.h>
#include "media/images_128_160.h"
...
void t_qtDisplay_MinerScreen(unsigned long mElapsed)
{
  ...
  Serial.printf(">>> Completed %s share(s), %s Khashes, avg. hashrate %s KH/s\n",
    ...
    //render.rdrawString(data.temp.c_str(), 90, 4, TFT_WHITE);

    //Hashrate
    render.setFontSize(16);
    render.setFontColor(TFT_WHITE);    
    render.rdrawString(data.currentHashRate.c_str(), 61, 99, TFT_BLACK);
    //background.setFreeFont(FF1);
    //background.setTextSize(1);
    //background.setTextColor(TFT_WHITE);
    //background.drawString("KH/s", 80, 105);
    
    //TotalHashes
    render.setFontSize(10);
    render.setFontColor(TFT_WHITE);    
    render.rdrawString(data.totalMHashes.c_str(), 150, 107, TFT_BLACK);

    //32Bit shares
    render.setFontSize(9);
    render.setFontColor(TFT_WHITE);    
    render.rdrawString(data.completedShares.c_str(), 85, 54, TFT_WHITE);

    //Valid Blocks
    render.setFontSize(20);
    render.drawString(data.valids.c_str(), 123, 40, TFT_SILVER);
    ...
    render.rdrawString(String(timeMining).c_str(), 155, 85, TFT_WHITE);
...
void t_qtDisplay_LoadingScreen(void)
{
  ...
  tft.drawString(CURRENT_VERSION, 24, 113, FONT2); 
}
